  ...
  public byte[] createResponse(InputStream inStream) {
    ...
    try {
      ...
      if (request == null) {
        ...
        if (request.length() == 0) {
          ...
          if (file.exists()) { // success
            ...
            try {
              ...
              builder.append("Content-Type: Integer; charset=utf-8\n");
              builder.append("\n");
              builder.append("No number found.");
            }
          ...
          try {
            ...
            builder.append(json.substring(json.indexOf("\"id"), json.indexOf("private")).replace(",", "\n"));

            builder.append("HTTP/1.1 200 OK\n");
            builder.append("Content-Type: text/html; charset=utf-8\n");
            builder.append("\n");
          } catch (Exception e) {
            builder.append("HTTP/1.1 404 Link Not Found\n");
            builder.append("Content-Type: text/html; charset=utf-8\n");
            builder.append("\n");
            builder.append("No github repository found");

          }
        } else if(request.contains("pythagorean?")) {
          try {
            Map<String, String> query_pairs = new LinkedHashMap<String, String>();
            // extract path parameters

            query_pairs = splitQuery(request.replace("pythagorean?", ""));
            // extract required fields from parameters
            Double side1 = Double.parseDouble(query_pairs.get("side1"));
            Double side2 = Double.parseDouble(query_pairs.get("side2"));

            // do math
            Double result = Math.sqrt((side1 * side1) + (side2 * side2));
            // Generate response
            builder.append("HTTP/1.1 200 OK\n");
            builder.append("Content-Type: Double; charset=utf-8\n");
            builder.append("\n");
            builder.append("Third side is: " + result);
          }catch(NumberFormatException n) {
            builder.append("HTTP/1.1 417 Expectation Failed\n");
            builder.append("Content-Type: Double; charset=utf-8\n");
            builder.append("\n");
        ...
        }else if(request.contains("volume_cylinder?")) {
          try {
            Map<String, String> query_pairs = new LinkedHashMap<String, String>();
            // extract path parameters

            query_pairs = splitQuery(request.replace("volume_cylinder?", ""));
            // extract required fields from parameters
            Double radius = Double.parseDouble(query_pairs.get("radius"));
            Double height = Double.parseDouble(query_pairs.get("height"));

            // do math
            Double result = radius * radius * height * Math.PI;
            // Generate response
            builder.append("HTTP/1.1 200 OK\n");
            builder.append("Content-Type: Double; charset=utf-8\n");
            builder.append("\n");
